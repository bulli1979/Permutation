...
public class PermTest {
	...
	public void testCount() {
		assertSame(Permutations.getLength(1), 1);
		assertSame(Permutations.getLength(2), 2);
		assertSame(Permutations.getLength(3), 6);
		assertSame(Permutations.getLength(4), 24);
	}
	...
	public void testPerm1() {
		int[][] result = Permutations.permutations(1);
		assertSame(1, result.length);
	}

	@Test
	public void testBuildBase() {
		int[] base = Permutations.buildBase(2);
		assertSame(base[1], 1);
		assertSame(base.length, 2);
	}
	...
	private void testPerm(int n) {
		...
		for (int[] perm : result) {
			...
			assertSame("Permutation has duplicates", n, (int) IntStream.of(perm).distinct().count());
			IntStream.of(perm).forEach(x -> assertTrue("Wrong permutation element", x >= 0 && x < n));
			List<Integer> permList = toList(perm);
			assertFalse("Permutation + " + Arrays.toString(perm) + " is generated twice", perms.contains(permList));
			perms.add(permList);
		}
	}
